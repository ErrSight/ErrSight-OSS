...
```
┌─ your app ──────────┐    POST /api/v1/events    ┌─ ErrSight Community ─┐
...
Open <http://localhost:3000>, sign up, and you're in. The first user gets a
personal organization auto-created; create a project, copy its API key, and
...
If you don't set the env vars, `db:seed` is a no-op; register the first
account through the regular sign-up form.
