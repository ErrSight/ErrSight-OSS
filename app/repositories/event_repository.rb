class EventRepository
  ...
  class << self
    ...
    # Per-fingerprint hourly counts for the issue-list sparklines. One grouped
    # query over the fingerprints already on the page; returns a fixed-length
    # array (oldest -> newest) per fingerprint so every sparkline has the same
    # number of bars. Buckets truncate at UTC to line up with the Ruby-side
    # keys (same approach as #time_series_counts).
    def spark_series_for(project:, fingerprints:, hours: 24)
      fingerprints = fingerprints.compact.uniq
      return {} if fingerprints.empty?

      # Anchor on the current hour and span `hours` buckets ending now, so the
      # in-progress hour is the last bar rather than being dropped off the end.
      since     = Time.current.utc.beginning_of_hour - (hours - 1).hours
      trunc_sql = "DATE_TRUNC('hour', occurred_at AT TIME ZONE 'UTC')"

      counts = Hash.new { |h, k| h[k] = Hash.new(0) }
      kept_for(project)
        .where(fingerprint: fingerprints)
        .where("occurred_at >= ?", since)
        .group(:fingerprint, Arel.sql(trunc_sql))
        .pluck(:fingerprint, Arel.sql(trunc_sql), Arel.sql("COUNT(*)"))
        .each { |fp, bucket, count| counts[fp][bucket.utc.strftime("%Y%m%d%H")] = count.to_i }

      keys = Array.new(hours) { |i| (since + i.hours).strftime("%Y%m%d%H") }
      fingerprints.index_with { |fp| keys.map { |k| counts[fp][k] } }
    end
