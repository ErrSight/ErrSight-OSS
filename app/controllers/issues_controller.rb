class IssuesController < ApplicationController
  ...
  def show
    ...
    # Issue rows in the list open a slide-in overlay via the #issue-overlay
    # Turbo Frame; serve the condensed detail for those requests and keep the
    # full triage + comments page for direct navigation and deep links.
    if turbo_frame_request?
      @latest_event    = @events.first
      @summary         = EventRepository.issue_summary(project: @project, fingerprint: @issue.fingerprint)
      @muted           = @project.mute_rules.exists?(fingerprint: @issue.fingerprint)
      @can_mute_events = policy(@project).mute_events?
      return render("overlay", layout: false)
    end
