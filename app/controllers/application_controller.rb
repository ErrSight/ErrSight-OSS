class ApplicationController < ActionController::Base
  ...
  def ensure_organization_exists!
    ...
    return if controller_path == "invitations"            # accept/show/decline handle membership themselves
    ...
    # An invitee who was already signed in when invited (a second tab, a
    # remembered session, or simply signing in before the invite arrived) never
    # hit after_sign_in_path_for's auto-accept, so they have a pending invite
    # but no membership — and would otherwise be bounced to the org-create form
    # on every request. Pull them into any org they've been invited to before
    # treating them as a brand-new user. Placed AFTER the controller guards so
    # it never pre-empts the explicit invitations#accept flow; only costs a
    # query for the rare no-org user (members short-circuit at the top).
    if Invitation.pending_for_email(current_user.email).exists?
      auto_accept_pending_invitations(current_user)
      return if current_user.organizations.kept.reload.any?
    end
