class Invitation < ApplicationRecord
  ...
  # How long a fresh — or re-sent — invitation stays valid.
  ...
  # Re-issue this invitation: refresh the validity window so the invitee gets a
  # fresh EXPIRY_WINDOW from now. Token, email, and role are unchanged, so the
  # original accept link still works — the caller just re-delivers the same
  # InvitationMailer.invite email.
  def refresh_expiry!
    update!(expires_at: EXPIRY_WINDOW.from_now)
  end
