class Issue < ApplicationRecord
  ...
  # Recomputes denormalized aggregate columns for the supplied
  # fingerprints by re-aggregating remaining events. Call this after a
  # bulk hard-delete (retention prune, storage-cap eviction, GDPR
  # erasure) — those paths use delete_all, which skips callbacks and
  # leaves the issues row pointing at events that no longer exist.
  #
  # Zero-event fingerprints get all aggregates zeroed in place rather
  # than the row deleted; the events#groups query filters
  # `occurrences_count > 0`, so they leave the list while comments /
  # assignment / external_url survive — if a future event re-activates
  # the fingerprint, the after_create_commit UPSERT writes back into
  # the same row.
  def self.reconcile_aggregates_for_fingerprints!(project_id:, fingerprints:)
    fps = Array(fingerprints).compact.uniq
    return if fps.empty?

    Issue.where(project_id: project_id, fingerprint: fps).find_each do |issue|
      events      = Event.kept.where(project_id: project_id, fingerprint: issue.fingerprint)
      most_recent = events.order(occurred_at: :desc).first

      if most_recent.nil?
        issue.issue_users.delete_all
      ...
      # Drop issue_users whose identifier no longer appears in any
      # remaining event for the issue. Without this, affected_users_count
      # below would refuse to decrement even after those users' events
      # were pruned — the same shape of drift the occurrences fix targets.
      remaining_users = events.where.not(user_identifier: [ nil, "" ])
                              .distinct.pluck(:user_identifier)
      stale = remaining_users.empty? ? issue.issue_users
                                     : issue.issue_users.where.not(user_identifier: remaining_users)
      stale.delete_all
      ...
      issue.update_columns(
        ...
        last_message:         most_recent.message,
        last_environment:     most_recent.environment,
        affected_users_count: issue.issue_users.count,
        updated_at:           Time.current
      )
    end
  end
