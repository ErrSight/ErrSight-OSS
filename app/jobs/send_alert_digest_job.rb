  ...
  def perform
    ...
    AlertPreference.where(digest_frequency: :hourly, email_enabled: true).includes(membership: :user).find_each do |preference|
      ...
      projects.each do |proj|
        ...
        # Atomically claim this hour's email digest for (preference, project) so a
        # mid-run failure + retry doesn't re-enqueue digests already sent this
        # hour. Mirrors the Slack digest claim in deliver_slack_digests.
        cache_key = "alert_digest_email:pref:#{preference.id}:project:#{proj.id}:hour:#{Time.current.beginning_of_hour.to_i}"
