...
Rails.application.configure do
  ...
  # Railway's filesystem is ephemeral — any local-disk upload is lost on deploy
  ...
  # Database-backed Rails.cache shared across Puma workers and Railway replicas.
  # Without this, every Rails.cache.write(..., unless_exist: true) used for alert
  # debouncing, quota-overage notification dedup, and cleanup gating is per-worker
  # — so the moment WEB_CONCURRENCY > 1, each worker thinks it's "first" and
  # customers get duplicate alerts. Settings live in config/cache.yml.
