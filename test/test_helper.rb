...
module TimescaleTestSupport
  ...
  # schema.rb does NOT record the timescaledb extension or re-run the hypertable
  # migration, so the events table in each worker DB starts as a plain table.
  ...
  def ensure_events_hypertable!
    ...
    # Enable the extension if its shared library is available — the
    # timescale/timescaledb image (docker-compose + CI) ships it, but a freshly
    # created test DB doesn't always inherit it from template1. On plain Postgres
    # the library is absent and this raises; rescue and leave events as a regular
    # table (the TimescaleDB-specific tests are the only ones that then differ).
    begin
