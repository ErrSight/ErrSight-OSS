...
class WebhookEndpointsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "viewer cannot see a webhook signing secret on the index" do
    endpoint = @project.webhook_endpoints.create!(url: "https://hooks.example.com/secret-check")
    sign_in @viewer
    get project_webhook_endpoints_path(@project)
    assert_response :success
  ...
  test "admin can see a webhook signing secret on the index" do
    endpoint = @project.webhook_endpoints.create!(url: "https://hooks.example.com/secret-check")
    sign_in @admin
    get project_webhook_endpoints_path(@project)
    assert_response :success
    assert_includes response.body, endpoint.secret
  end
