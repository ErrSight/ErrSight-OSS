...
class IssuesControllerTest < ActionDispatch::IntegrationTest
  ...
  # ── slide-in overlay (Turbo Frame) ──────────────────────────────────────────

  test "GET issue as a Turbo Frame request renders the overlay, not the full triage page" do
    get project_issue_path(@project, @event.fingerprint),
      headers: { "Turbo-Frame" => "issue-overlay" }

    assert_response :success
    assert_match(/<turbo-frame[^>]*id="issue-overlay"/, response.body)
    assert_select ".iss-ov-head"
    assert_match(/STACK TRACE/, response.body)
    # Triage + comments belong to the full page, not the quick-look overlay.
    assert_no_match(/Discussion/, response.body)
  end

  test "overlay offers a View events link to this issue's events" do
    get project_issue_path(@project, @event.fingerprint),
      headers: { "Turbo-Frame" => "issue-overlay" }

    assert_response :success
    assert_select ".iss-ov-actions a[href=?]",
      project_events_path(@project, fingerprint: @event.fingerprint, q: "is:all"),
      text: "View events"
  end

  test "overlay renders the fix-prompt accordion in the context column" do
    get project_issue_path(@project, @event.fingerprint),
      headers: { "Turbo-Frame" => "issue-overlay" }

    assert_response :success
    assert_select ".iss-ov-cols .fix-prompt-panel details.fix-prompt > summary", text: /Fix prompt/i
    assert_select "details.fix-prompt pre[data-fix-prompt-target=?]", "content", text: /You are debugging/
    assert_select "details.fix-prompt button[data-action=?]", "click->fix-prompt#copy", text: /Copy to clipboard/
    # No longer a modal trigger in the action bar.
    assert_select ".iss-ov-actions button[data-action='click->fix-prompt#open']", false
    assert_select "dialog.fix-prompt-dialog", false
  end

  test "GET issue without a Turbo Frame renders the full triage + comments page" do
    get project_issue_path(@project, @event.fingerprint)

    assert_response :success
    assert_match(/Triage/, response.body)
    assert_match(/Discussion/, response.body)
    assert_no_match(/<turbo-frame[^>]*id="issue-overlay"/, response.body)
  end
end
