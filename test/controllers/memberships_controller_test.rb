...
class MembershipsControllerTest < ActionDispatch::IntegrationTest
  ...
  # ── pending invitations: resend ──────────────────────────────────────────────

  test "admin sees a Resend button on pending invitations" do
    invitation = @org.invitations.create!(email: "waiting@example.com", role: :member, invited_by: @admin)
    sign_in @admin

    get organization_memberships_path(@org)
    assert_response :success
    assert_select "form[action=?] button", resend_organization_invitation_path(@org, invitation),
      text: "Resend"
  end

  test "viewer does not see resend or revoke controls on pending invitations" do
    invitation = @org.invitations.create!(email: "waiting2@example.com", role: :member, invited_by: @admin)
    sign_in @viewer

    get organization_memberships_path(@org)
    assert_response :success
    # Viewer still sees the pending invite listed, but no admin-only action forms.
    assert_match "waiting2@example.com", response.body
    assert_select "form[action=?]", resend_organization_invitation_path(@org, invitation), count: 0
  end
