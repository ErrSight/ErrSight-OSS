...
class ProjectsAndEventsTest < ApplicationSystemTestCase
  ...
  test "the Resolved saved view surfaces resolved events" do
    # Saved-view tabs only render in layouts A/C; pin layout A so the test does
    # not depend on a leftover `items_layout` cookie from a prior ?layout= test.
    visit project_events_path(@project, layout: "a")
