...
class OauthSignInTest < ApplicationSystemTestCase
  setup do
    ...
    # ErrSight OSS defaults to invite-only (ALLOW_PUBLIC_SIGNUP=false), which
    # makes the OAuth callback block brand-new accounts. These tests exercise
    # the open-signup path, so flip the gate on (the app server shares this
    # process's ENV) and restore it in teardown.
