...
class EventRepositoryTest < ActiveSupport::TestCase
  ...
  test "prune_older_than! zeros issue aggregates so ghost rows leave the groups list" do
    # Drives the bug the user reported: pre-fix, the issues row kept its
    # stale occurrences_count and the fingerprint kept surfacing on the
    # groups page even though every backing event had been hard-deleted.
    @project.events.create!(
      level: :error, message: "doomed", fingerprint: "fp-ghost",
      occurred_at: 10.days.ago, size_bytes: 500
    )

    issue = Issue.find_by!(project: @project, fingerprint: "fp-ghost")
    assert_equal 1, issue.occurrences_count

    EventRepository.prune_older_than!(project_id: @project.id, cutoff: 3.days.ago)

    issue.reload
    assert_equal 0, issue.occurrences_count
    assert_equal 0, issue.resolved_count

    groups = EventRepository.grouped_by_fingerprint(project_id: @project.id)
    refute_includes groups.map(&:fingerprint), "fp-ghost"
  end

  test "prune_older_than! recomputes aggregates when only some events for a fingerprint are pruned" do
    @project.events.create!(
      level: :warning, message: "old occurrence", fingerprint: "fp-partial",
      occurred_at: 10.days.ago, size_bytes: 100
    )
    @project.events.create!(
      level: :error, message: "recent occurrence", fingerprint: "fp-partial",
      occurred_at: 1.day.ago, size_bytes: 200
    )

    issue = Issue.find_by!(project: @project, fingerprint: "fp-partial")
    ...
    assert_equal "recent occurrence", issue.last_message
  end
  ...
  test "delete_oldest_for_project! zeroes issue aggregates for fully evicted fingerprints" do
    @project.events.create!(
      level: :error, message: "evicted", fingerprint: "fp-evict",
      occurred_at: 10.days.ago, size_bytes: 100
    )

    EventRepository.delete_oldest_for_project!(project_id: @project.id, limit: 1)

    issue = Issue.find_by!(project: @project, fingerprint: "fp-evict")
    assert_equal 0, issue.occurrences_count
  end
  ...
  test "erase_by_user_identifier! drops issue_users and decrements affected_users_count" do
    @project.events.create!(
      level: :error, message: "shared", fingerprint: "fp-erase",
      occurred_at: 1.hour.ago, user_identifier: "user-a", size_bytes: 100
    )
    @project.events.create!(
      level: :error, message: "shared", fingerprint: "fp-erase",
      occurred_at: 2.hours.ago, user_identifier: "user-b", size_bytes: 100
    )

    issue = Issue.find_by!(project: @project, fingerprint: "fp-erase")
    assert_equal 2, issue.affected_users_count

    EventRepository.erase_by_user_identifier!(
      project_id: @project.id, user_identifier: "user-a"
    )

    issue.reload
    assert_equal 1, issue.affected_users_count
    refute issue.issue_users.exists?(user_identifier: "user-a")
    assert issue.issue_users.exists?(user_identifier: "user-b")
  end
  ...
  # -- Sparkline series (issue-list mini charts) -----------------------------

  test "spark_series_for returns an empty hash when given no fingerprints" do
    assert_equal({}, EventRepository.spark_series_for(project: @project, fingerprints: []))
  end

  test "spark_series_for returns a fixed-length bucket array for each fingerprint" do
    s = EventRepository.spark_series_for(project: @project, fingerprints: %w[a b], hours: 24)
    assert_equal %w[a b], s.keys
    assert(s.values.all? { |bars| bars.length == 24 }, "every series should have one bar per hour")

    s6 = EventRepository.spark_series_for(project: @project, fingerprints: %w[a], hours: 6)
    assert_equal 6, s6["a"].length, "the hours arg controls the number of bars"
  end

  test "spark_series_for counts a current-hour event in the final bucket" do
    # Regression guard for the off-by-one where the in-progress hour fell off
    # the end of the window and recent events vanished from the sparkline.
    @project.events.create!(level: :error, message: "x", fingerprint: "fp-recent",
                            occurred_at: Time.current.utc.beginning_of_hour, size_bytes: 100)
    bars = EventRepository.spark_series_for(project: @project, fingerprints: %w[fp-recent], hours: 24)["fp-recent"]
    assert_equal 1, bars.sum, "the event is counted exactly once"
    assert_equal 1, bars.last, "a current-hour event belongs in the final (most recent) bucket"
  end

  test "spark_series_for excludes events outside the window and zero-fills quiet fingerprints" do
    @project.events.create!(level: :error, message: "old", fingerprint: "fp-old",
                            occurred_at: 3.days.ago, size_bytes: 100)
    series = EventRepository.spark_series_for(project: @project, fingerprints: %w[fp-old fp-silent], hours: 24)
    assert_equal 0, series["fp-old"].sum, "events older than the window are excluded"
    assert_equal 0, series["fp-silent"].sum, "a fingerprint with no events is all zeros"
  end

  test "spark_series_for is scoped to the project" do
    other = projects(:beta)
    other.events.create!(level: :error, message: "x", fingerprint: "fp-cross",
                         occurred_at: 10.minutes.ago, size_bytes: 100)
    bars = EventRepository.spark_series_for(project: @project, fingerprints: %w[fp-cross], hours: 24)["fp-cross"]
    assert_equal 0, bars.sum, "an event in another project must not leak into this project's sparkline"
  end
end
